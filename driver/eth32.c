...
const eth_errinfo_t eth_errinfo[] = {
    ...
    {ETH_NETWORK_ERROR,
     "ETH_NETWORK_ERROR: Network communications error.  Connection was unable "
     "to be established or existing connection was broken."},
    {ETH_THREAD_ERROR, "ETH_THREAD_ERROR: Internal error occurred in the "
                       "threads and synchronization library."},
    {ETH_NOT_SUPPORTED,
     "ETH_NOT_SUPPORTED: Function not supported by this platform or device."},
    {ETH_PIPE_ERROR,
     "ETH_PIPE_ERROR: Internal API error dealing with data pipes."},
    {ETH_RTHREAD_ERROR, "ETH_RTHREAD_ERROR: Internal API error dealing with "
                        "the \"Reader thread.\""},
    {ETH_ETHREAD_ERROR, "ETH_ETHREAD_ERROR: Internal API error dealing with "
                        "the \"Event thread.\""},
    {ETH_MALLOC_ERROR,
     "ETH_MALLOC_ERROR: Error dynamically allocating memory."},
    {ETH_WINDOWS_ERROR, "ETH_WINDOWS_ERROR: Internal API error specific to the "
                        "Microsoft Windows platform."},
    {ETH_WINSOCK_ERROR, "ETH_WINSOCK_ERROR: Internal API error in dealing with "
                        "the Microsoft Winsock library."},
    {ETH_NETWORK_INTR,
     "ETH_NETWORK_INTR: Network read/write operation was interrupted."},
    {ETH_WRONG_MODE, "ETH_WRONG_MODE: Something is not configured correctly in "
    ...
    {ETH_CFG_NOACK, "ETH_CFG_NOACK: Warning - device did not acknowledge our "
                    "attempt to store a new configuration."},
    {ETH_CFG_REJECT,
     "ETH_CFG_REJECT: Device has rejected the new configuration data we "
     "attempted to store.  Configuration switch on device may be disabled."},
    {ETH_LOADLIB, "ETH_LOADLIB: Error loading an external DLL library."},
    {ETH_PLUGIN, "ETH_PLUGIN: General error with the currently configured "
                 "plugin/sniffer library."},
    {ETH_BUFSIZE,
    ...
    {ETH_INVALID_CHANNEL, "ETH_INVALID_CHANNEL: The given channel number does "
                          "not exist on this device."},
    {ETH_INVALID_POINTER, "ETH_INVALID_POINTER: The pointer passed in to an "
                          "API function was invalid."},
    {ETH_INVALID_OTHER, "ETH_INVALID_OTHER: One of the parameters passed in to "
                        "an API function was invalid."},
    {ETH_INVALID_VALUE, "ETH_INVALID_VALUE: The given value is out of range "
                        "for this I/O port, counter, etc."},
...
#ifdef WINDOWS
BOOL CALLCONVENTION DllMain(HINSTANCE hDllInst, DWORD fdwReason,
                            LPVOID lpvReserved) {
...
eth32 CALLCONVENTION eth32_open(char *address, WORD port, unsigned int timeout,
                                int *result) {
  // timeout specifies, in milliseconds, how long the connection attempt may
  // take, excluding resolving DNS if a non-numeric address is given. Specify a
  // timeout of zero to indicate using the system's timeout.
  eth32_data *data;
  ...
  // These aren't critical, but give some reasonable defaults for the
  // event_handler fields if the user is to do a get_event_handler and inspect
  // them.
  data->event_handler.maxqueue =
      CALLBACK_DEFAULT; // max queue size for callback events

  // Launch a thread dedicated to reading from the socket
  if (wth_thread_create(eth32_readthread, (void *)data,
                        &(data->readthread_handle))) {
  ...
  return (data); /* return a pointer to the structure we just allocated and
                    initialized */
...
int CALLCONVENTION eth32_output_bit(eth32 handle, int port, int bit,
                                    int value) {
...
int CALLCONVENTION eth32_pulse_bit(eth32 handle, int port, int bit, int edge,
                                   int count) {
...
int CALLCONVENTION eth32_input_successive(eth32 handle, int port, int max,
...
int CALLCONVENTION eth32_input_bit(eth32 handle, int port, int bit,
                                   int *value) {
  int v;
  int res;
...
int CALLCONVENTION eth32_set_direction_bit(eth32 handle, int port, int bit,
                                           int direction) {
...
int CALLCONVENTION eth32_get_direction_bit(eth32 handle, int port, int bit,
                                           int *direction) {
...
int CALLCONVENTION eth32_set_analog_reference(eth32 handle, int reference) {
  ...
  if (reference != REF_EXTERNAL && reference != REF_INTERNAL &&
      reference != REF_256)
    return (ETH_INVALID_OTHER);
...
int CALLCONVENTION eth32_set_analog_eventdef(eth32 handle, int bank,
                                             int channel, int lomark,
                                             int himark, int defaultval) {
  ...
  if (defaultval != ANEVT_DEFAULT_LOW && defaultval != ANEVT_DEFAULT_HIGH)
    return (ETH_INVALID_OTHER);
...
int CALLCONVENTION eth32_get_analog_eventdef(eth32 handle, int bank,
                                             int channel, int *lomark,
                                             int *himark) {
...
int CALLCONVENTION eth32_set_analog_assignment(eth32 handle, int channel,
                                               int source) {
...
int CALLCONVENTION eth32_get_analog_assignment(eth32 handle, int channel,
                                               int *source) {
...
int CALLCONVENTION eth32_reset(eth32 handle) {
  ...
  // Leave the event handler configuration untouched so that all the user has to
  // do to re-subscribe to an event is call enable_event.
...
int CALLCONVENTION eth32_get_serialnum_string(eth32 handle, char *serial,
                                              int bufsize) {
  ...
  // a dash and a NULL.
  // The eth32cfg_serialnum_string function will do the final check, but we want
  // to return an error as quickly as possible if we know ahead of time the
  // buffer is too small.
...
int CALLCONVENTION eth32_get_firmware_release(eth32 handle, int *major,
                                              int *minor) {
...
int CALLCONVENTION eth32_set_event_queue_config(eth32 handle, int maxsize,
                                                int fullqueue) {
...
int CALLCONVENTION eth32_get_event_queue_status(eth32 handle, int *maxsize,
                                                int *fullqueue, int *cursize) {
...
int CALLCONVENTION eth32_dequeue_event(eth32 handle, eth32_event *event,
                                       int timeout) {
  ...
  //  timeout:  positive - max number of milliseconds to wait before returning
  //  TIMEOUT
  ...
  res = 0;
  check = 0;
  while (handle->evt_queue->count == 0 && handle->evt_queue_size > 0 &&
         !(check = eth32_check(handle, 0))) {
  ...
  if (check) // Problem with the reading thread, so we won't be getting any more
             // data.
  {
    retval = check;
    goto exit_label;
...
int CALLCONVENTION eth32_enable_event(eth32 handle, int type, int port, int bit,
                                      int id) {
  ...
  case EVENT_DIGITAL:
    ...
    if (bit == -1) // Port event
    {
      ...
      bitmask = 0xff;
    } else { // bit must be from 0 through 7
...
int CALLCONVENTION eth32_disable_event(eth32 handle, int type, int port,
                                       int bit) {
...
int CALLCONVENTION eth32_set_event_handler(eth32 handle,
                                           eth32_handler *handler) {
...
int CALLCONVENTION eth32_get_event_handler(eth32 handle,
                                           eth32_handler *handler) {
  // This function simply returns the currnetly-stored event handler
  // configuration
...
int CALLCONVENTION eth32_set_counter_state(eth32 handle, int counter,
                                           int state) {
  ...
  if (state != COUNTER_DISABLED && state != COUNTER_FALLING &&
...
int CALLCONVENTION eth32_get_counter_state(eth32 handle, int counter,
                                           int *state) {
...
int CALLCONVENTION eth32_set_counter_value(eth32 handle, int counter,
                                           int value) {
...
int CALLCONVENTION eth32_get_counter_value(eth32 handle, int counter,
                                           int *value) {
...
int CALLCONVENTION eth32_set_counter_rollover(eth32 handle, int counter,
                                              int rollover) {
...
int CALLCONVENTION eth32_get_counter_rollover(eth32 handle, int counter,
                                              int *rollover) {
...
int CALLCONVENTION eth32_set_counter_threshold(eth32 handle, int counter,
                                               int threshold) {
...
int CALLCONVENTION eth32_get_counter_threshold(eth32 handle, int counter,
                                               int *threshold) {
...
int CALLCONVENTION eth32_set_pwm_clock_state(eth32 handle, int state) {
  ...
  if (state != PWM_CLOCK_DISABLED && state != PWM_CLOCK_ENABLED)
    return (ETH_INVALID_OTHER);
...
int CALLCONVENTION eth32_set_pwm_channel(eth32 handle, int channel, int state) {
  ...
  if (state != PWM_CHANNEL_DISABLED && state != PWM_CHANNEL_NORMAL &&
...
int CALLCONVENTION eth32_get_pwm_channel(eth32 handle, int channel,
                                         int *state) {
...
int CALLCONVENTION eth32_set_pwm_duty_period(eth32 handle, int channel,
                                             int period) {
...
int CALLCONVENTION eth32_get_pwm_duty_period(eth32 handle, int channel,
                                             int *period) {
...
int CALLCONVENTION eth32_set_pwm_parameters(eth32 handle, int channel,
                                            int state, float freq, float duty) {
  // This function performs a variety of tasks in order to configure a PWM
  // channel It configures the PWM clock, the base frequency, the channel state,
  // the channel duty cycle, and puts the corresponding I/O pin into output
  // mode. All of this functionality could be done manually by calling the
  // appropriate functions. Note that if a channel is enabled, the I/O pin used
  // for the output will be put into output mode.  However, if a channel is
  // disabled, the direction will not be affected. NOTE:  All PWM signals on a
  // board share the same frequency.  Therefore,
  ...
  //     duty:    duty cycle ratio (percentage) specified as a fraction between
  //     0 and 1
...
int CALLCONVENTION eth32_get_pwm_parameters(eth32 handle, int channel,
                                            int *state, float *freq,
                                            float *duty) {
  ...
  // This function simply calls other publicly accessible functions, which are
  // already protected with the reference counter, so we don't need to deal with
  // the reference counter in here.
  if ((res = eth32_check(handle, 0)))
    return (res);
...
int CALLCONVENTION CALLEXTRA eth32_get_eeprom(eth32 handle, int address,
                                              int length, void *buffer) {
  // This function retrieves data from the user-accessible EEPROM memory
  // on-board the ETH32. address specifies the starting address in the EEPROM.
  // "length" bytes will be stored into the user-provided buffer pointed to by
  // buffer.
...
int CALLCONVENTION CALLEXTRA eth32_set_eeprom(eth32 handle, int address,
                                              int length, void *buffer) {
  // This function stores data into the user-accessible EEPROM memory on-board
  // the ETH32. address specifies the starting address in the EEPROM. "length"
  // bytes will be stored and most be provided in the buffer.
